...
import { Settings, CreditCard, DollarSign, Building, Sparkles, Save, Plus, Trash2, ShieldCheck, UserPlus, UserX, Mail, KeyRound, Send, CheckCircle2 } from 'lucide-react';
...
export function SettingsManagement() {
  ...
  // New admin OTP creation state
  const [newAdmin, setNewAdmin] = useState({ username: '', email: '', password: '', otp: '' });
  ...
  const handleRequestAdminOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await adminApi.requestNewAdminOtp(token, {
        username: newAdmin.username,
        email: newAdmin.email,
      });
      if (response.success) {
        toast.success(`Verification code sent to ${newAdmin.email}`);
  ...
  const handleCreateAdmin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newAdmin.username || !newAdmin.email || !newAdmin.password || !newAdmin.otp) {
      toast.error('Please enter all fields including the 6-digit verification code');
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        ...
        setNewAdmin({ username: '', email: '', password: '', otp: '' });
    ...
    <AdminLayout>
      <div className="space-y-6 max-w-5xl mx-auto">
        ...
        <div className={`p-6 rounded-2xl border shadow-xl space-y-6 text-xs transition-colors ${
          ...
          {/* Add New Admin Form — 2-Step Email Verification */}
          <div className={`pt-4 border-t space-y-4 ${isLight ? 'border-slate-200' : 'border-slate-800'}`}>
            <div className="flex items-center justify-between">
              <h4 className={`font-semibold uppercase tracking-wider text-[10px] ${isLight ? 'text-slate-900' : 'text-white'}`}>
                Add New Administrator (Email Verified)
              </h4>
              {adminOtpSent && (
                <span className="text-[10px] font-bold text-emerald-500 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-0.5 rounded-full inline-flex items-center gap-1">
                  <CheckCircle2 className="w-3 h-3" /> OTP Sent to Email
                </span>
              )}
            </div>
              ...
              <form onSubmit={handleRequestAdminOtp} className="space-y-4">
                <div className="grid md:grid-cols-3 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. admin_sujata"
                      className={`w-full rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:border-cyan-500 border ${
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="admin@rksmahila.org"
                        className={`w-full pl-9 rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:border-cyan-500 border ${
                          isLight ? 'bg-white border-slate-300 text-slate-900' : 'bg-slate-900 border-slate-800 text-white'
                        }`}
                        required
                      />
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className="px-5 py-2.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-bold flex items-center gap-2 shadow-lg transition-all disabled:opacity-50 text-xs"
                  >
                    <Send className="w-3.5 h-3.5" />
                    {isSendingAdminOtp ? 'Sending Verification Code...' : 'Send Verification OTP to Email'}
                  </button>
                </div>
              </form>
            ) : (
              <form onSubmit={handleCreateAdmin} className="space-y-4">
                <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-xl p-3 text-xs text-cyan-400">
                  📧 We've sent a 6-digit verification code to <strong className="text-white">{newAdmin.email}</strong>. Please enter the OTP code below to finalize admin creation.
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-3 items-end">
                  <div>
                    ...
                    <div className="relative">
                      <KeyRound className="w-4 h-4 text-cyan-500 absolute left-3 top-3" />
                      <input
                        type="text"
                        maxLength={6}
                        value={newAdmin.otp}
                        onChange={(e) => setNewAdmin({ ...newAdmin, otp: e.target.value })}
                        placeholder="Enter 6-digit OTP code"
                        className={`w-full pl-9 rounded-xl px-4 py-2.5 text-xs font-mono font-bold tracking-widest focus:outline-none focus:border-cyan-500 border ${
                          isLight ? 'bg-white border-slate-300 text-slate-900' : 'bg-slate-900 border-slate-800 text-white'
                        }`}
                        required
                      />
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex-1 px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold flex items-center justify-center gap-2 shadow-lg transition-all disabled:opacity-50 text-xs"
                    >
                      <UserPlus className="w-4 h-4" />
                      {isCreatingAdmin ? 'Verifying & Creating Admin...' : 'Verify OTP & Create Administrator'}
                    </button>
                    ...
                    <button
                      ...
                      className={`px-3 py-2.5 rounded-xl border text-xs font-semibold ${
                        isLight ? 'border-slate-300 hover:bg-slate-100 text-slate-700' : 'border-slate-800 hover:bg-slate-900 text-slate-300'
                      }`}
                    >
                      Back
                    </button>
